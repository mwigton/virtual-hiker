...
enum image_output_enum {add = 0,subtract = 1,change = 2,multiply = 3,devide = 4,difference = 5,average = 6,max = 7,min = 8,content = 9}
...
class preimage_edit_class
{
	...
	function GetPixelRaw(buffer: buffer_class,x: long,y: long): Color
	{
		...
		if (x < 0) {x = 0;}
		else if (x > buffer.outerRect.width-1) {x = buffer.outerRect.width-1;}
		if (y < 0) {y = 0;}
		else if (y > buffer.outerRect.height-1) {y = buffer.outerRect.height-1;}
