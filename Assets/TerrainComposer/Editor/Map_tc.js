...
class Map_tc extends EditorWindow
{
	...
	function OnGUI()
	{
		...
		*/
						...
						if (global_script.map.export_heightmap_active)
						{
							for (var count_elExt: int = 0;count_elExt < global_script.map.elExt.Count;++count_elExt)
							{
								...
								if (global_script.map.elExt[count_elExt].error) {
									global_script.DrawRect(Rect(-offset_map.x+export_p1.x,offset_map.y+export_p1.y,width_p1,height_p1),Color.red,2,Rect(0,0,position.width,position.height)); 
								}
								else {
		...
		EditorGUI.DrawPreviewTexture(Rect(0,0,1270,24),global_script.tex2);
		...
		if (global_script.map.button_image_editor)
		{	
			image_editor_rect = Rect(0,gui_y,guiWidth2+348,109+((global_script.map.preimage_edit.edit_color.Count*19)+(Convert.ToInt32(current_area.preimage_save_new)*60)+gui_height));
			...
			for (var count_edit_color: int = 0;count_edit_color < global_script.map.preimage_edit.edit_color.Count;++count_edit_color) {
				...
				if (global_script.map.preimage_edit.edit_color[count_edit_color].output != image_output_enum.content) {
					...
					if (GUI.changed) {
						...
						else if (global_script.map.preimage_edit.radiusSelect > 740) {global_script.map.preimage_edit.radiusSelect = 740;}	
					}
		...
		if (global_script.map.button_update)
	    {
			...
			else if (global_script.settings.update_version && global_script.settings.wc_loading == 0)
			{
				if (GUI.Button(wc_gui.getRect(1,4,80,18,false,true),"Download"))
				{
					content_version();
			...
			{
				if (GUI.Button(wc_gui.getRect(1,4,80,19,false,true),"Import"))
				{
		...
		{	
			...
			if (global_script.map.key_edit)
			{
				...
				for (var countKey: int = 0;countKey < global_script.map.bingKey.Count;++countKey)
				{
					...
					GUI.color = global_script.map.backgroundColor; 
					EditorGUI.DrawPreviewTexture(Rect(guiWidth2+5,(countKey*19)+gui_y-99-(global_script.map.bingKey.Count*19),global_script.get_label_width("Key"+countKey.ToString()+" -> '"+global_script.map.bingKey[countKey].key+"'",true),17),global_script.tex2);
					GUI.color = Color.red;
					EditorGUI.LabelField(Rect(guiWidth2+5,(countKey*19)+gui_y-99-(global_script.map.bingKey.Count*19),position.width,50),"Key"+countKey.ToString()+" -> '"+global_script.map.bingKey[countKey].key+"'",EditorStyles.boldLabel);
					GUI.color = global_script.map.color;
				}
				wc_gui.x = 0;
		...
		if (global_script.map.button_region)
		{
			...
			*/
			...
			if (current_region.area.Count >  0)
			{
				...
				{
					if (key.shift) {
						if (!current_area.export_heightmap_active && !current_area.export_image_active) {
							#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
		...
		{	
			wc_gui.x = 0;
			wc_gui.y = gui_y+23;
			heightmap_export_rect = Rect(0,gui_y,guiWidth2+1,165+gui_height);
		...
		{	
			...
			if (global_script.map.export_raw){gui_height += 19;}
			...
			if (GUI.Button(wc_gui.getRect(1,4,25,16,true,false),"+",EditorStyles.miniButtonMid))
			{
				if (!current_area.export_image_active) {
			...
			if (GUI.changed)
			{
				...
				else if (current_area.resolution > 8192){current_area.resolution = 8192;}
			...
			EditorGUI.LabelField(wc_gui.getRect(1,4,15,19,true,false),"X");
			...
			if (global_script.map.path_display) {
				GUI.color = global_script.map.backgroundColor;
				EditorGUI.DrawPreviewTexture(Rect(image_export_rect.x+guiWidth2+15,image_export_rect.y+97,global_script.get_label_width(current_area.export_image_path,true),20),global_script.tex2);
				GUI.color = global_script.map.color;
				EditorGUI.LabelField(Rect(image_export_rect.x+guiWidth2+15,image_export_rect.y+98,global_script.get_label_width(current_area.export_image_path,true),20),GUIContent(current_area.export_image_path),EditorStyles.boldLabel);
			}
			...
			GUI.color = Color.white;
			current_area.export_image_import_settings = EditorGUI.Toggle(wc_gui.getRect(1,2,25,19,false,false),current_area.export_image_import_settings);
			...
			if (global_script.map.export_raw)
			{
				...
				if (global_script.map.export_jpg)
				{
					...
					{
						slice_textures(current_area,current_area.export_image_path,current_area.export_image_filename);
					}
				}
				else if (GUI.Button(wc_gui.getRect(0,0,120,19,false,true),"Combine Images"))
				{
					combine_textures(current_area,current_area.export_image_path,current_area.export_image_filename);
				}
			}
		...
		{	
				...
				settings_rect = Rect(0,gui_y,guiWidth2+1,122+gui_height);
		...
		*/
		...
		if (global_script.map.button_create_terrain) {
			...
			*/
			...
			GUI.changed = gui_changed_old;
			GUI.color = global_script.map.color;
			EditorGUI.LabelField(Rect(guiWidth2+427,186+gui_y2+import_height,200,20),"("+(current_area.heightmap_resolution.x/current_area.tiles.x).ToString()+")");
			...
			if ((global_script.map.export_image.subtile_total == global_script.map.export_image.subtiles_total) || (export_image_area.resolution == 512))
			{
				...
				if (global_script.map.export_jpg) {
					...
					if (export_image_area.preimage_export_active && export_image_area.preimage_save_new) {
						global_script.map.preimage_edit.y1 = 0;
						global_script.map.preimage_edit.convert_texture (global_script.map.tex2,global_script.map.tex2,global_script.map.tex2.width,global_script.map.tex2.height,false);
						export_texture_as_jpg(export_image_area.preimage_path+"/"+export_image_area.preimage_filename+".jpg",global_script.map.tex2,global_script.map.export_jpg_quality);
					}
				}
				...
				if (global_script.map.export_png) {
					...
					if (export_image_area.preimage_export_active && export_image_area.preimage_save_new) {
						global_script.map.preimage_edit.y1 = 0;
						global_script.map.preimage_edit.convert_texture (global_script.map.tex2,global_script.map.tex2,global_script.map.tex2.width,global_script.map.tex2.height,false);
						export_texture_to_file(export_image_area.export_image_path,export_image_area.preimage_filename,global_script.map.tex2);
					}
				}
				
				if (global_script.map.export_raw) {
					export_texture_to_raw(export_image_area.export_image_path,filename,global_script.map.tex2);
					
					if (export_image_area.preimage_export_active && export_image_area.preimage_save_new) {
						global_script.map.preimage_edit.y1 = 0;
						global_script.map.preimage_edit.convert_texture (global_script.map.tex2,global_script.map.tex2,global_script.map.tex2.width,global_script.map.tex2.height,false);
						export_texture_to_raw(export_image_area.export_image_path,export_image_area.preimage_filename,global_script.map.tex2);
					}
				}
	...
	*/
	...
	function export_texture_to_raw(path: String,file: String,export_texture: Texture2D)
	{
		var export_file: FileStream;
		var pixels2: Color[];
		var y: int;
		var x: int;
		var width: int = export_texture.width;
		var height: int = export_texture.height;
		var byte1: byte[] = new byte[width*3];
		
		export_file = new FileStream(path+"/"+file+".raw",FileMode.Create);
		
		for (y = 0;y < height;++y) {
			pixels2 = export_texture.GetPixels(0,height-y-1,width,1);
			
			for (x = 0;x < width;++x) {
				byte1[x*3] = pixels2[x].r*255;
				byte1[(x*3)+1] = pixels2[x].g*255;
				byte1[(x*3)+2] = pixels2[x].b*255;
			}
			
			export_file.Write(byte1,0,width*3);
		}
		
		export_file.Close();
	}
	
	function combine_textures(area1: map_area_class,path: String,file: String) 
	{
		var current_tile: tile_class = new tile_class();
		var import_filename: String;
		var import_path: String = area1.export_image_path+"/";
		
		var import_file: FileStream;
		var export_file: FileStream;
		
		var byte1: byte[] = new byte[area1.resolution*3];
		var y: int;
		var x: int;
		var y1: int;
		var x1: int;
		
		var width: ulong = area1.resolution*3*area1.tiles.x;
		var height: ulong = width*area1.resolution;
		var length: ulong = ((area1.resolution*area1.resolution*3))*(area1.tiles.x*area1.tiles.y);
		
		export_file = new FileStream(path+"/"+file+"_combined.raw",FileMode.Create);
		export_file.SetLength (length);
		
		for (y = 0;y < area1.tiles.y;++y) {
			for (x = 0;x < area1.tiles.x;++x) {
				import_filename = area1.export_image_filename+"_x"+x+"_y"+y;
				import_file = new FileStream(import_path+import_filename+".raw",FileMode.Open);
				
				export_file.Seek ((height*y)+(area1.resolution*3*x),SeekOrigin.Begin);
				
				for (y1 = 0;y1 < area1.resolution;++y1) {
					// for (x1 = 0;x1 < area1.resolution*3;++x1) {
						import_file.Read (byte1,0,area1.resolution*3);
						export_file.Write (byte1,0,area1.resolution*3);
					// }
					export_file.Seek (width-(area1.resolution*3),SeekOrigin.Current);
				}
				import_file.Close();
			}
		}
	...
	function slice_textures(area1: map_area_class,path: String,file: String) 
	{
		var current_tile: tile_class = new tile_class();
		var import_filename: String;
		var export_path: String = area1.export_image_path+"/";
		...
		var width: long = area1.resolution*3*area1.tiles.x;
		var height: long = width*area1.resolution;
		var length: long = ((area1.resolution*area1.resolution*3))*(area1.tiles.x*area1.tiles.y);
		
		if (!global_script.map.preimage_edit.save_new) {
			import_file = new FileStream(path+"/"+file+"_combined2.raw",FileMode.Open);
		}
		else {
			import_file = new FileStream(current_area.preimage_path+"/"+current_area.preimage_filename+".raw",FileMode.Open);
		}
		var pixels2: Color[] = new Color[area1.resolution];
		...
		for (y = 0;y < area1.tiles.y;++y) {
			for (x = 0;x < area1.tiles.x;++x) {
				export_filename = area1.export_image_filename+"_x"+x+"_y"+y;
				// import_file = new FileStream(import_path+import_filename+".raw",FileMode.Open);
				
				import_file.Seek ((height*y)+(area1.resolution*3*x),SeekOrigin.Begin);
				
				for (y1 = 0;y1 < area1.resolution;++y1) {
					import_file.Read (byte1,0,area1.resolution*3);
					for (x1 = 0;x1 < area1.resolution;++x1) {
						// import_file.Read(byte1,0,area1.resolution*3);
						pixels2[x1] = Color((byte1[(x1*3)]*1.0)/255,(byte1[(x1*3)+1]*1.0)/255,(byte1[(x1*3)+2]*1.0)/255);
					}
					global_script.map.tex2.SetPixels(0,area1.resolution-y1-1,area1.resolution,1,pixels2);
					import_file.Seek (width-(area1.resolution*3),SeekOrigin.Current);
				}
				export_texture_as_jpg(export_path+export_filename+".jpg",global_script.map.tex2,global_script.map.export_jpg_quality);
				// import_file.Close();
				// return;
			}
		}
	...
	*/
	...
	{
		conversion_step.x = (heightmap_resolution*terrain_region.area[0].tiles_select.x)/(raw_file.resolution.x-1);
		conversion_step.y = (heightmap_resolution*terrain_region.area[0].tiles_select.y)/(raw_file.resolution.y-1);
		
		// Debug.Log(conversion_step);
	...
	function calc_raw_value(pos: Vector2,offset: Vector2): float
	{
		...
		// Debug.Log("x: "+h_local_x+" y: "+h_local_y);
		...
		var byte2: byte = raw_file.fs.ReadByte();
	   	...
	   	return (((byte1*raw_file.product1)+(byte2*raw_file.product2))/65536.0);
	}
